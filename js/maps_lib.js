(function (window, undefined) {
    var MapsLib = function (options) {
        ...
        self.getgeoCondition = function (address, callback) {
            if (address !== "") {
        ...
        self.doSearch = function (location, extraClauses) {
            self.clearSearch();
            extraClauses = extraClauses || '';
            var address = $("#search_address").val();
            self.searchRadius = $("#search_radius").val();
            self.whereClause = self.locationColumn + " not equal to ''";
            self.whereClause += extraClauses;
            //-----custom filters-------

            var type_column = "'Program Type'";
            var tempWhereClause = [];
            if ( $("#cbType1").is(':checked')) tempWhereClause.push("Family Child Care");
            if ( $("#cbType2").is(':checked')) tempWhereClause.push("Large Group and School Age Child Care");
    ...
    MapsLib.prototype.reset = function () {
        this.clearSearch();
        this.map.setOptions(this.myOptions);
        this.submitSearch(this.locationColumn + " not equal to ''", this.map);
    };
